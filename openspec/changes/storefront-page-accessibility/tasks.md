...
- [ ] 4.3 Write a test asserting that ordering one item and then a second item, in a row, lets focus move from the first item's control to the second item's control without returning to the top of the page.
- [ ] 4.4 Change `order()`/`loadItems()` in `app/index.html` so the item just ordered keeps or regains focus on one of its own controls after the list re-renders.
