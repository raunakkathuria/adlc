...
Someone using a screen reader hears the catalogue announce itself: an empty search result, a stock count that changed after an order, and the size of the list itself, all read automatically instead of silently — concisely, as a count of what changed rather than a recitation of every item, and once a search settles rather than once per keystroke. Every item's own details are still there to find — navigating into the list surfaces them — they are just not read out unprompted as part of the change announcement. They can navigate the catalogue as a list, the way a sighted user sees it as one, and that structure holds regardless of how the page styles the list. A keyboard user who places an order keeps their place afterward instead of landing back at the top of the page. And no item's name — however it's spelled — can break out of the page's markup or run as script.
