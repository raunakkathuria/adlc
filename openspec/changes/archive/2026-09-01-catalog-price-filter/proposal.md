...
A `max_price` that isn't a non-negative whole number of cents — including an empty value (e.g. `?max_price=`) or the same parameter supplied more than once — is refused with a `400` and a reason, the same way the rest of this API refuses bad input — it never fails silently, never picks one of several supplied values, and never falls back to "show everything."
...
- Changing how a repeated `q` (`?q=a&q=b`) is handled. It is accepted silently today, an accident of how the query string is read rather than a decision anyone made — that is shipped behaviour, and revisiting it belongs in its own change with its own Gate 1.
