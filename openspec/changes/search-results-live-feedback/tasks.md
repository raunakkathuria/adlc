...
- [ ] 1.2 `REQ-CAT-7`: the page's automatic search on load writes the exact wording `Showing 1 item.` (catalogue holds exactly one item) or `Showing {n} items.` (catalogue holds `{n}` items, `{n}` more than one) into the summary
...
- [ ] 1.8 `REQ-CAT-7`: placing an order, accepted or rejected, refreshes the item list under the same query but leaves the summary's content unchanged and triggers no new announcement for it
- [ ] 1.9 `REQ-CAT-8`: stub `fetch` in the page-script sandbox so an earlier query's response resolves after a later query's; assert the later query's results render and the earlier response never overwrites them
- [ ] 1.10 `REQ-CAT-8`: a single, settled search (no overlapping request) still renders and announces normally
- [ ] 1.11 Confirm every new test fails against today's page script before touching it
...
- [ ] 2.3 Distinguish a user-typed or page-load search from the item-list refresh that follows placing an order, so the latter never rewrites or re-announces the summary (`REQ-CAT-7`)
- [ ] 2.4 Track which query is the most recently issued and discard any response that arrives for an earlier one, so it is never rendered or announced (`REQ-CAT-8`)
