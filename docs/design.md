...
3. **On/off is one switch.** A credential secret present → the line runs unattended, end to end. Absent → every workflow runs, explains itself, and stops (documentation mode). Either credential counts: `ADLC_API_KEY` for API billing, or `CLAUDE_CODE_OAUTH_TOKEN` from `claude setup-token` for a Claude subscription. The key is named for this line rather than for a vendor, because it also authenticates against a gateway; `scripts/run-station.sh` maps it to whatever the pinned CLI reads.
