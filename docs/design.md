...
- **REQ-ORD-3 and the stock check disagree, and nothing noticed for four days.** The requirement says an order over 20 units is rejected "regardless of stock", and its scenario says *when 21 units are ordered, the response is `422 {"reason":"over_limit"}`* — naming no SKU, so it quantifies over every item. `app/server.mjs` checks stock first, so `PEN-1` (stock 8) answers `insufficient_stock` and the scenario's THEN is false for it. Either the requirement records that the stock check takes precedence, or the two guards swap. Not decided here: `AGENTS.md` says a contradiction found mid-build is routed to the Planner, not settled by whoever is holding the keyboard. Filed as an issue once the app-stability PR is in, so the line runs it against a clean `main`.
