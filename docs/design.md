...
- **231 tests passed while the happy path was fatally broken.** A helper was renamed and one call site was missed, so every otherwise-successful local build threw `ReferenceError` after pushing its branch and before opening its PR — and the whole suite stayed green, because the house rule is to export the pure decision and test that, which leaves the orchestration between those decisions covered by nothing. `npm run verify` cannot see it either: the gate runs the product's tests, not the driver's wiring. An independent reviewer found it by reading the diff. The structural guard for this class is a linter — a dangling reference is exactly what `no-undef` exists for — and this repo had none. **It does now**, and the argument that settled it is worth keeping: an attempt to substitute a hand-rolled scanner for one *failed silently*. Run against the broken commit it reported an unrelated identifier, not `vendorOf`, because its template-literal stripping regex over-consumed and deleted the region containing the bug — a tool that answers "clean" because it threw away what it was meant to check is worse than no tool, and it is the shape a regex approximation of scope analysis will always tend to. `oxlint` through `npx`, the same route `quality.yml:152` already uses for Lighthouse, adds nothing to `package.json` and is measured on the only test that matters: against `6aecf16:local/build.mjs` it reports exactly one finding, `'vendorOf' is not defined` at 434:51, and exits 1. It sits in `npm run verify`, so a dangling reference now parks a build in both drivers instead of reaching `main`.

- **Twice, a fix traded one defect for a smaller one instead of removing the class.** The once-only parking flag fixed a duplicated comment and introduced a lost park. The whole-line verdict scan fixed "APPROVE / REQUEST CHANGES — undecided" reading as approval and introduced a rejection of any review whose *reason* mentioned the other verdict — which rejected, among other things, a review discussing this parser. Both were caught by the next round, and both second attempts were narrower than the first: park stages tracked individually, and the verdict read from the decision field rather than the whole line. The pattern to watch for is a fix that adds a condition rather than sharpening the rule — the condition is where the new failure hides, and the sharper rule is usually available one thought later. The third attempt broke the habit: instead of a fourth condition on the parking flags, the question "which stages still owe?" became a pure function with five tests, and the remaining verdict edge was written down rather than patched. Three defects in six lines is what it took to stop reaching for a condition; the tell was that every one of the three was about *which calls still owed*, never about the calls.

- **A rule stated once in a shared prompt gets implemented twice, and the copies drift.** The prompts are the single definition of each station and both drivers run them — which is the design's strength, and it quietly creates a second place for every rule that needs *reading* as well as writing. `prompts/review.md` asks the reviewer for one verdict line; enforcing that meant a check in `local/build.mjs` and a check in `build.yml`, written the same afternoon by the same author from the same sentence. They disagreed within the hour: `APPROVED — looks good` passed in CI and parked locally, because one had a word boundary and the other did not. Three of the four twin-fidelity defects found in this driver were this shape — the same intent, implemented twice, differing in a detail neither implementation's tests could see. The answer is already in the repo and was not reached for: `labels.mjs`, `attempts.mjs`, `links.mjs` and now `review-verdict.mjs` exist precisely so a rule has one reader, and both drivers call it. **Anything a prompt tells a station to emit in a fixed shape belongs in `scripts/` the moment a second driver needs to parse it** — not in whichever driver needs it first.

- **Three false claims about the same tool, from three different kinds of inference.** First: "OpenSpec has no RENAMED operation", inferred from grepping this repo's deltas, which only ever use ADDED/MODIFIED/REMOVED — absence of use read as absence of support. Second, in the correction: "whitespace normalised, so spacing is forgiving", inferred from the *name* of `normalizeRequirementName()`, whose body is `return name.trim()` — the ends only, so a double space inside a requirement name is still a different requirement. Third, in the same correction: that a scenario could be "deliberately removed and replaced", inferred from the existence of REMOVED at the requirement level — `findMissingCurrentScenarios()` rejects every existing scenario name omitted from a MODIFIED block, and there is no scenario-level removal at all. Each correction introduced the next error, and each error was one indirection away from the truth: the repo's usage, a function's name, an operation's existence at a different level. Then a fourth, of a different kind: having read the source and correctly established that there is **no scenario-level operation**, the prompt went on to say the delta "cannot say it" — false, because OpenSpec's operations work on whole requirements, so REMOVED plus ADDED replaces a requirement and its scenarios with it. The fact checked was true; the sentence written was a generalisation of it that had not been checked. So the rule has two halves: **a claim about a tool's behaviour is verified by reading the code that implements it, and nothing else counts** — not the repo's habits, not an identifier, not an analogy from a neighbouring feature — **and the claim you verify has to be the claim you write.** Four errors, four indirections, every one of them a step the author did not notice taking. A prompt is documentation as much as instruction, and a confidently wrong sentence in one teaches every future run to work around a limitation that does not exist.

- **A prompt asserted something false about a tool, and the check was inference rather than the tool.** `prompts/spec.md` gained a rule saying `openspec archive` matches headings character for character and that OpenSpec has no RENAMED operation. Both were wrong: names are normalised, and `## RENAMED Requirements` with `FROM:`/`TO:` lines has existed all along. The error came from grepping *this repo's* deltas — which only ever use ADDED, MODIFIED and REMOVED — and generalising from their absence, instead of reading the installed parser. The rule was written to fix a real failure and would have taught the Planner to work around a limitation that does not exist. Two lessons, and the second is the useful one: a prompt is documentation of a tool's behaviour as much as an instruction, so a claim about a tool gets verified against the tool; and the repo's own usage is evidence of what it does, never of what is possible.
