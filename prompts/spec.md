...
**A MODIFIED block must carry the living spec's wording, not just its id.** `openspec archive` finds the requirement you are modifying by its whole `### Requirement:` heading (whitespace normalised, so spacing is forgiving and wording is not), and it refuses a block that appears to drop a `#### Scenario:` the living requirement already has — it cannot tell a rename from a deletion. So carry the existing heading and every existing scenario name through unchanged, and add your new scenarios alongside them.
...
Use it when a heading has become wrong, and MODIFIED alongside it for the body. **There is no equivalent for scenarios** — a scenario name cannot be renamed, only kept, added or removed, so a scenario whose name no longer fits either keeps it or is deliberately removed and replaced, and the proposal says which.
...
5. **Say how it composes.** If the new behaviour interacts with something that exists, say what happens when both apply. Somebody will hit that combination on day one. And when you put an existing piece of data somewhere new, every guarantee it already carries goes with it — find them in the living spec, name them, and say each still holds or why it does not. A query that had to be inert in one message is not allowed to be markup in the next one just because the requirement that said so was written for somewhere else.
6. **Name what you are not doing.** An explicit out-of-scope list is what stops the delta growing during the build. Before you leave something out because an earlier change already left it out, read why. A scope-out is a decision someone took against the product as it was, and this change may be the thing that expires it — a message excluded because its content came from a fixed set stops being excluded the moment your delta routes user data through it. Cite the earlier proposal and either say its reason still holds, or bring the thing in.
